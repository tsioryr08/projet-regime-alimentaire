...
<head>
  ...
  <title>Admin — Créer un régime</title>
...
<body>
  <div class="app">
    <aside class="sidebar">
      ...
      <a href="<?= site_url('admin/regimes') ?>" class="nav-item active"><svg viewBox="0 0 24 24"><path d="M12 2v20"/><path d="M17 6H9.5a3.5 3.5 0 0 0 0 7H14a3.5 3.5 0 0 1 0 7H7"/></svg>Régimes</a>
    ...
    <main class="main">
      <div class="topbar"><div class="topbar-title">Créer un régime</div><div class="topbar-actions"><a class="btn btn-secondary btn-sm" href="<?= site_url('admin/regimes') ?>">Retour</a></div></div>
      <div class="content">
        <div class="form-card">
          <form method="post" action="<?= site_url('admin/regimes/create') ?>">
            ...
            <div class="form-grid cols-3">
              <div><label class="field-label" for="prix_base">Prix de base</label><input id="prix_base" name="prix_base" type="number" step="0.01" placeholder="Prix de base"></div>
              <div><label class="field-label" for="duree_jours">Durée (jours)</label><input id="duree_jours" name="duree_jours" type="number" placeholder="Durée jours"></div>
              <div><label class="field-label" for="variation_poids">Variation de poids</label><input id="variation_poids" name="variation_poids" type="number" step="0.01" placeholder="Variation poids"></div>
              <div><label class="field-label" for="sens_variation">Sens variation</label><input id="sens_variation" name="sens_variation" placeholder="prise/perte"></div>
              <div><label class="field-label" for="pct_viande">% viande</label><input id="pct_viande" name="pct_viande" type="number" step="0.01" placeholder="% viande"></div>
              <div><label class="field-label" for="pct_poisson">% poisson</label><input id="pct_poisson" name="pct_poisson" type="number" step="0.01" placeholder="% poisson"></div>
              <div><label class="field-label" for="pct_volaille">% volaille</label><input id="pct_volaille" name="pct_volaille" type="number" step="0.01" placeholder="% volaille"></div>
            </div>
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:12px;"><a class="btn btn-secondary" href="<?= site_url('admin/regimes') ?>">Annuler</a><button type="submit" class="btn btn-primary">Enregistrer</button></div>
          </form>
        </div>
      </div>
    </main>
