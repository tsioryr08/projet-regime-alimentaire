...
<head>
  ...
  <title>Admin — Validation code</title>
...
<body>
  <div class="app">
    ...
    <main class="main">
      <div class="topbar"><div class="topbar-title">Validation code</div><div class="topbar-actions"><a class="btn btn-secondary btn-sm" href="<?= site_url('admin/codes') ?>">Retour</a></div></div>
      <div class="content">
        <div class="form-card">
          <form method="post" action="<?= site_url('admin/codes/validation/' . ($code['id'] ?? 0)) ?>">
            <?= csrf_field() ?>
            <div class="form-section-title">Code sélectionné</div>
            <div class="form-grid cols-1">
              <div><label class="field-label">Code</label><input value="<?= esc($code['code'] ?? '') ?>" disabled></div>
              <div><label class="field-label" for="est_valide">Statut</label><select id="est_valide" name="est_valide"><option value="1" <?= ((int) ($code['est_valide'] ?? 1) === 1) ? 'selected' : '' ?>>Valide</option><option value="0" <?= ((int) ($code['est_valide'] ?? 1) === 0) ? 'selected' : '' ?>>Invalide</option></select></div>
            </div>
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:12px;">
              <a class="btn btn-secondary" href="<?= site_url('admin/codes') ?>">Annuler</a>
